...
import { fetchTripUpdates, fetchAlerts, fetchVehiclePositions } from "../lib/gtfsrt";

const router = Router();

const TTL_TRIPS  = 30_000;
const TTL_ALERTS = 60_000;
...
router.get("/trips", async (_req: Request, res: Response) => {
  ...
  try {
    ...
    await cacheSet(CACHE_KEY, data, TTL_TRIPS);
    res.json(data);
  } catch (err) {
...
router.get("/alerts", async (_req: Request, res: Response) => {
  ...
  try {
    ...
    await cacheSet(CACHE_KEY, data, TTL_ALERTS);
    res.json(data);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
...
const TTL_VEHICLES = 30_000;

// GET /v1/realtime/vehicles
router.get("/vehicles", async (_req: Request, res: Response) => {
  const CACHE_KEY = "realtime:vehicles";
  const cached = await cacheGet<object>(CACHE_KEY);
  if (cached) return res.json(cached);

  try {
    const data = await fetchVehiclePositions();
    await cacheSet(CACHE_KEY, data, TTL_VEHICLES);
    res.json(data);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("[realtime/vehicles]", message);
    res.status(502).json({ error: "upstream_error", message, status: 502 });
  }
});
